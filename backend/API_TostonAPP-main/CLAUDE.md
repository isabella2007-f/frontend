...
- **Precio del domicilio = snapshot del barrio** — sale de `Barrios.Precio` + ofertas del día, se congela en `Domicilios` al crear el pedido y **no se recalcula** (ni pendientes ni históricos). `COSTO_DOMICILIO` fue eliminada. Ver "Módulo Ubicaciones" abajo. No sumar un costo fijo de domicilio en ningún lado. **Pedido dividido**: un domicilio por cada viaje a domicilio, con snapshot propio por grupo.
...
- **Snapshot**: al crear el pedido se congela en `Domicilios` (`ID_Barrio`, `Precio_Domicilio_Base`, `Precio_Domicilio_Final`, `Desglose_Ofertas` JSON). No se recalcula aunque después cambie el precio del barrio o una oferta. `Venta.Total` usa el snapshot. **Pedido dividido = un domicilio por viaje**: al partir el pedido en grupos de envío (`crear_grupos_envio`), cada grupo a domicilio congela **su propio** snapshot (`_domicilio_grupo_kwargs`: su barrio o el heredado + ofertas de la fecha de ESE grupo) y `Venta.Total` se recalcula (`Total − dom_original + Σ dom_grupos`). El `Domicilio` con `ID_Grupo IS NULL` queda de referencia y deja de aportar al total. `actualizar_tipo_entrega_grupo` (domicilio↔tienda) y `cancelar_grupo_pendiente` ajustan `Venta.Total` por el domicilio del grupo. Respuesta: `grupos_envio[].precio_domicilio_final` + `costo_domicilio_total`.
