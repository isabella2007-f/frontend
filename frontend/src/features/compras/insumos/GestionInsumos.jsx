...
function VerInsumo({ ins, onClose }) {
  const { getCatInsumo, getUnidad } = useApp();
  const cat    = getCatInsumo(ins.idCategoria);
  const unidad = getUnidad(ins.idUnidad);
  const dias   = ins.lote ? diasParaVencer(ins.lote.fechaVenc) : null;
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box modal-box--lg" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <div><p className="modal-header__eyebrow">Gestión de Insumos</p><h2 className="modal-header__title">{ins.nombre}</h2></div>
          <button className="modal-close-btn" onClick={onClose}>✕</button>
        </div>
        <div className="modal-body">
          <div className="detail-grid" style={{ marginBottom: 14 }}>
            <div><p className="detail-field__label">ID Insumo</p><p className="detail-field__value" style={{ color: "#9e9e9e" }}>#{String(ins.id).padStart(4,"0")}</p></div>
            <div><p className="detail-field__label">Categoría</p><span className="cat-chip"><span className="cat-chip__icon">{cat.icon}</span>{cat.nombre}</span></div>
            <div><p className="detail-field__label">Unidad de medida</p><span className="unidad-badge">{unidad.simbolo} — {unidad.nombre}</span></div>
            <div><p className="detail-field__label">Estado operativo</p><Toggle value={ins.estado} onChange={() => {}} /></div>
          </div>
          <hr className="detail-divider" />
          <p style={{ margin: "0 0 10px", fontSize: 12, fontWeight: 700, color: "#9e9e9e", textTransform: "uppercase", letterSpacing: "0.8px" }}>Stock</p>
          <div className="detail-grid" style={{ marginBottom: 14 }}>
            <div><p className="detail-field__label">Stock actual</p><p className="detail-field__value" style={{ fontSize: 22, color: "#2e7d32" }}>{ins.stockActual}</p></div>
            <div><p className="detail-field__label">Stock mínimo</p><p className="detail-field__value" style={{ fontSize: 22, color: "#9e9e9e" }}>{ins.stockMinimo}</p></div>
            <div style={{ gridColumn: "1 / -1" }}><StockBar actual={ins.stockActual} minimo={ins.stockMinimo} /></div>
          </div>
          {ins.lote && (
            <>
              <hr className="detail-divider" />
              <div className="lote-section">
                <div className="lote-section__header"><span>📦</span><p className="lote-section__title">Lote — {ins.lote.id}</p></div>
                <div className="lote-section__body">
                  <div><p className="lote-field__label">Fecha vencimiento</p><p className="lote-field__value">{ins.lote.fechaVenc}</p></div>
                  <div><p className="lote-field__label">Días restantes</p><p className="lote-field__value" style={{ color: dias <= 30 ? "#e65100" : "#2e7d32" }}>{dias}d</p></div>
                  <div><p className="lote-field__label">Cantidad inicial</p><p className="lote-field__value">{ins.lote.cantInicial}</p></div>
                </div>
              </div>
            </>
          )}
...
export default function GestionInsumos() {
  ...
  const filtered = insumos.filter(ins => {
    ...
    const cat = getCatInsumo(ins.idCategoria);
    const matchQ   = ins.nombre.toLowerCase().includes(q) || cat.nombre.toLowerCase().includes(q) || ins.lote?.id?.toLowerCase().includes(q);
  ...
  // Todas las categorías (activas + inactivas) para el filtro de la tabla
  const todasCats = categoriasInsumos;
    ...
    <div className="page-wrapper">
      ...
      <div className="page-inner">
        ...
        <div className="stats-row">
          <div className="stat-card"><div className="stat-card__icon" style={{ background: "#e8f5e9" }}>🧺</div><div><div className="stat-card__num">{insumos.length}</div><div className="stat-card__label">Total insumos</div></div></div>
          <div className="stat-card"><div className="stat-card__icon" style={{ background: "#e8f5e9" }}>✅</div><div><div className="stat-card__num" style={{ color: "#2e7d32" }}>{statsDisp}</div><div className="stat-card__label">Disponibles</div></div></div>
          <div className="stat-card"><div className="stat-card__icon" style={{ background: "#fff8e1" }}>⚠️</div><div><div className="stat-card__num" style={{ color: "#f57f17" }}>{statsBajo}</div><div className="stat-card__label">Stock bajo</div></div></div>
          <div className="stat-card"><div className="stat-card__icon" style={{ background: "#ffebee" }}>🚫</div><div><div className="stat-card__num" style={{ color: "#c62828" }}>{statsAgot}</div><div className="stat-card__label">Agotados</div></div></div>
        </div>
        ...
        <div className="toolbar">
          <div className="search-wrap">
            ...
            <input type="text" className="search-input" placeholder="Buscar por nombre, categoría o lote…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div ref={filterRef} style={{ position: "relative" }}>
            ...
            {showFilter && (
              <div className="filter-dropdown" style={{ minWidth: 170 }}>
                ...
                <button className={`filter-option${filterCat === "todas" ? " active" : ""}`} onClick={() => setFilterCat("todas")}>
                  <span className="filter-dot" style={{ background: "#bdbdbd" }} />Todas
                </button>
                {todasCats.map(c => (
                  <button key={c.id} className={`filter-option${filterCat === c.id ? " active" : ""}`} onClick={() => setFilterCat(c.id)}>
                ...
                {filterEstOptions.map(f => (
                  <button key={f.val} className={`filter-option${filterEst === f.val ? " active" : ""}`} onClick={() => { setFilterEst(f.val); setShowFilter(false); }}>
        ...
        <div className="card">
          <div style={{ overflowX: "auto" }}>
            <table className="tbl">
              <thead><tr><th>Nº</th><th>Insumo</th><th>Categoría</th><th>Unidad</th><th>Stock</th><th>Activo</th><th>Acciones</th></tr></thead>
              <tbody>
                {paginated.length === 0 ? (
                  <tr><td colSpan={7}><div className="empty-state"><div className="empty-state__icon">🧺</div><p className="empty-state__text">Sin insumos encontrados</p></div></td></tr>
                ) : paginated.map((ins, idx) => {
                  const cat    = getCatInsumo(ins.idCategoria);
                  const unidad = getUnidad(ins.idUnidad);
                  return (
                    <tr key={ins.id} className="tbl-row">
                      <td><span style={{ fontSize: 12, fontWeight: 600, color: "#bdbdbd" }}>{String((safePage - 1) * ITEMS_PER_PAGE + idx + 1).padStart(2, "0")}</span></td>
                      <td><div className="insumo-name-cell"><span className="insumo-name">{ins.nombre}</span><span className="insumo-id">#{String(ins.id).padStart(4,"0")}</span></div></td>
                      <td><span className="cat-chip"><span className="cat-chip__icon">{cat.icon}</span>{cat.nombre}</span></td>
                      ...
                      <td>
                        <div className="actions-cell">
                          <button className="act-btn act-btn--view"   onClick={() => setModal({ type: "ver",      ins })}>👁</button>
                          <button className="act-btn act-btn--edit"   onClick={() => setModal({ type: "editar",   ins })}>✎</button>
                          <button className="act-btn act-btn--delete" onClick={() => setModal({ type: "eliminar", ins })}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
          ...
          <div className="pagination-bar">
            <span className="pagination-count">{filtered.length} {filtered.length === 1 ? "insumo" : "insumos"} en total</span>
            <div className="pagination-btns">
              <button className="pg-btn-arrow" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={safePage === 1}>‹</button>
              <span className="pg-pill">Página {safePage} de {total}</span>
              <button className="pg-btn-arrow" onClick={() => setPage(p => Math.min(total, p + 1))} disabled={safePage === total}>›</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {toast && (
        <div className="toast" style={{ background: toast.type === "error" ? "#c62828" : "#2e7d32" }}>
          <span className="toast-icon">{toast.type === "error" ? "🗑️" : "✅"}</span>{toast.msg}
        </div>
      )}
    </div>
  );
