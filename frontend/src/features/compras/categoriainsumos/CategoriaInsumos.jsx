...
function InsumosPills({ insumos }) {
  const MAX = 3; const rest = insumos.length - MAX;
  return (
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="modal-header">
        <div><p className="modal-header__eyebrow">Categoría de insumos</p><h2 className="modal-header__title">Detalle</h2></div>
        <button className="modal-close-btn" onClick={onClose}>✕</button>
      </div>
      <div className="modal-body">
        <div className="form-group"><label className="form-label">Ícono</label><span style={{ fontSize: 32 }}>{cat.icon}</span></div>
        <div className="form-group"><label className="form-label">Nombre</label><div className="field-input field-input--disabled">{cat.nombre}</div></div>
        <div className="form-group"><label className="form-label">Descripción</label><div className="field-input field-input--disabled" style={{ minHeight: 60 }}>{cat.descripcion}</div></div>
        <div className="form-group">
          <label className="form-label">Insumos ({cat.insumos.length})</label>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{cat.insumos.map(i => <span key={i} className="insumo-pill">{i}</span>)}</div>
        </div>
        <div className="form-group" style={{ marginBottom: 0 }}>
          <label className="form-label">Estado</label>
          <span className="status-pill" style={{ background: cat.estado ? "#e8f5e9" : "#f5f5f5", color: cat.estado ? "#2e7d32" : "#9e9e9e", border: `1px solid ${cat.estado ? "#a5d6a7" : "#e0e0e0"}` }}>
            <span className="status-dot" style={{ background: cat.estado ? "#43a047" : "#bdbdbd" }} />{cat.estado ? "Activo" : "Inactivo"}
          </span>
        </div>
        {cat.fecha && <div className="date-info"><span>📅</span><span>Creada el <strong>{cat.fecha}</strong></span></div>}
      </div>
...
export default function CategoriaInsumos() {
  ...
  const filtered = categoriasInsumos.filter(c => {
    ...
    return (c.nombre.toLowerCase().includes(q) || c.descripcion.toLowerCase().includes(q) || c.insumos.some(i => i.toLowerCase().includes(q))) &&
      ...
      <div className="page-inner">
        <div className="toolbar">
          <div className="search-wrap">
            ...
            <input type="text" className="search-input" placeholder="Buscar por nombre, descripción o insumo…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        <div className="card">
          <div style={{ overflowX: "auto" }}>
            <table className="tbl">
              <thead><tr><th>Nº</th><th>Categoría</th><th>Descripción</th><th>Insumos</th><th>Estado</th><th>Fecha</th><th>Acciones</th></tr></thead>
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  <tr key={cat.id} className="tbl-row">
                    ...
                    <td><div className="cat-cell"><div className="cat-icon-box">{cat.icon}</div><span className="cat-name">{cat.nombre}</span></div></td>
                    <td style={{ maxWidth: 200 }}><span className="cat-desc">{cat.descripcion}</span></td>
                    <td><InsumosPills insumos={cat.insumos} /></td>
                    <td><Toggle value={cat.estado} onChange={() => toggleCatInsumo(cat.id)} /></td>
                    <td><span className="cat-date">{cat.fecha}</span></td>
                    <td>
                      <div className="actions-cell">
                        <button className="act-btn act-btn--view"   onClick={() => setModal({ type: "ver",      cat })}>👁</button>
                        <button className="act-btn act-btn--edit"   onClick={() => setModal({ type: "editar",   cat })}>✎</button>
                        <button className="act-btn act-btn--delete" onClick={() => setModal({ type: "eliminar", cat })}>🗑️</button>
                      </div>
                    </td>
                  </tr>
                ))}
